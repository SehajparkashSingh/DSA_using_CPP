...
void interleave(queue *q){       //uses only QUEUE operations 
    if(q->present_sz%2!=0)
        return;
    queue *s=create_queue(q->present_sz/2);
    int size=q->present_sz;
...
void interleave1(queue *q){      //uses QUEUE and STACK operations 
    if(q->present_sz%2!=0)         // Eg.: 1 2 3 4 5 6
        return;
    stack *s=create_stack(q->present_sz/2);
    int size=q->present_sz;
    for(int i=0;i<size/2;i++){
        push(s,dequeue(q));       // 1 2 3
    }
    for(int i=0;i<size/2;i++){
        enqueue(q,pop(s));        //4 5 6 3 2 1
    }
    for(int i=0;i<size/2;i++){
        enqueue(q,dequeue(q));    //3 2 1 4 5 6
    }
    for(int i=0;i<size/2;i++){
        push(s,dequeue(q));       //3 2 1
    }
    for(int i=0;i<size/2;i++){
        enqueue(q,pop(s));
        enqueue(q,dequeue(q));
...
int main(){
    ...
    enqueue(q,6);
    print_queue(q);
    interleave(q);    //size should be 'even'
    print_queue(q);
    interleave1(q);    //size should be 'even'
    print_queue(q);
}